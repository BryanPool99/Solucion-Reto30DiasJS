...
let max=0;
for (const key of Object.values(users)) {
    if(key.skills.length>max){
        max=key.skills.length;
    }
}
for (let i = 0; i < Object.values(users).length; i++) {
    if(Object.values(users)[i].skills.length==max){
...
*/
const objV=Object.values(users);
let contUC=0,contMC=0;
for (let i = 0; i < objV.length; i++) {
    if(objV[i].isLoggedIn) contUC++;
    if(objV[i].points>50) contMC++;
}
console.log("Usuarios conectados: "+contUC);
console.log("Usuarios que tienen mas de 50 puntos :"+contMC);
/*
...
const idxA=[];
for (let i = 0; i < objV.length; i++) {
    if( objV[i].skills.includes('MongoDB') &&
        objV[i].skills.includes('Express') &&
        objV[i].skills.includes('React') &&
        objV[i].skills.includes('Node')
    ){
        idxA.push(i);
        contIdx++;
    }
...
const keyC=Object.values(countries);
